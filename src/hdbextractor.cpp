                           ...
                           const char *stop_date)
{
    ...
    if(!success && d_ptr->dbschema)
        snprintf(d_ptr->errorMessage, MAXERRORLEN, "Hdbextractor.getData: %s", d_ptr->dbschema->getError());
    else if(!success && d_ptr->connection)
        snprintf(d_ptr->errorMessage, MAXERRORLEN, "Hdbextractor.getData: database is not available: connection: %s",
                 d_ptr->connection->getError());
    else if(!success)
        snprintf(d_ptr->errorMessage, MAXERRORLEN, "Hdbextractor.getData: database is not available");
