...
MySqlHdbppSchema::MySqlHdbppSchema(ResultListener *resultListenerI) : ConfigurableDbSchema()
{
    ...
    memset(d_ptr->errorMessage, 0, MAXERRORLEN);
            ...
            if(!d_ptr->isCancelled)
            {
                ...
                if(wri == XVariant::RO)
                {
                    ...
                    if(format == XVariant::Vector)
                        snprintf(query, MAXQUERYLEN, "SELECT data_time,value_r,dim_x_r,idx,quality,att_error_desc_id FROM "
                        ...
                        snprintf(query, MAXQUERYLEN, "SELECT data_time,value_r,quality,att_error_desc_id FROM "
                                                     " %s WHERE att_conf_id=%d AND data_time >='%s' "
...
//                            }
                ...
                else if(wri == XVariant::RW)
                {
                    ...
                    if(format == XVariant::Vector)
                        snprintf(query, MAXQUERYLEN, "SELECT data_time, value_r, %s dim_x_r, idx, quality,att_error_desc_id FROM "
                                                     " %s WHERE att_conf_id=%d AND data_time >='%s' "
                                                     " AND data_time <= '%s' ORDER BY data_time,idx ASC",
                                 column_value_w, table_name, id, start_date, stop_date);
                    else
                        snprintf(query, MAXQUERYLEN, "SELECT data_time,value_r, %s quality,att_error_desc_id FROM "
                                                     " %s WHERE att_conf_id=%d AND  data_time >='%s' "
                                                     " AND data_time <= '%s' ORDER BY data_time ASC",
                                 column_value_w, table_name, id, start_date, stop_date);
                                      ...
                                      ConfigurableDbSchemaHelper::FillFromThePastMode mode)
{
    ...
    {
        snprintf(query, MAXQUERYLEN, "SELECT data_time,dim_x_r,idx,value_r,quality,att_error_desc_id FROM "
    ...
    {
        snprintf(query, MAXQUERYLEN, "SELECT data_time, 1 AS dim_x_r, 0 AS idx,value_r,quality,att_error_desc_id FROM "
                                     " %s WHERE att_conf_id=%d AND data_time <= "
    ...
    {
        snprintf(query, MAXQUERYLEN, "SELECT data_time, 1 AS dim_x_r, 0 AS idx,value_r, %s quality,att_error_desc_id FROM "
                                     " %s WHERE att_conf_id=%d AND data_time  <= "
                                     " '%s' ORDER BY data_time DESC LIMIT 1",
